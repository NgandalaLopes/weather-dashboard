...
$('.search').on("click", function (event) {
	...
	city = $(this).parent('.btnBind').siblings('.wrote').val().trim();
	if (city === "") {
		return;
	};
	pastRecord.push(city);

	localStorage.setItem('city', JSON.stringify(pastRecord));
	forecastCardsEl.empty();
	getpastRec();
	getWeatherToday();
});

//Will create buttons based on search pastRec 
var contRecEl = $('.pastRecord');
function getpastRec() {
	contRecEl.empty();
	...
	for (let i = 0; i < pastRecord.length; i++) {
		...
		var rowEl = $('<row>');
		var btnEl = $('<button>').text(`${pastRecord[i]}`)

		rowEl.addClass('row recButton');
		btnEl.addClass('btn btn-outline-secondary histBtn');
		btnEl.attr('type', 'button');

		contRecEl.prepend(rowEl);
		rowEl.append(btnEl);
	} if (!city) {
		return;
	}
...
var cardTodayBody = $('.todayWeather')
...
function getFiveDayForecast() {
	...
	$.ajax({
		...
		for (let i = 0; i < myWeather.length; i++) {
			...
			forecastCardsEl.append(divElCard);
...
function initLoad() {
	...
	var pastRecordStore = JSON.parse(localStorage.getItem('city'));

	if (pastRecordStore !== null) {
		pastRecord = pastRecordStore
	}
	getpastRec();
	getWeatherToday();
};
